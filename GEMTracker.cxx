...
GEMTracker::GEMTracker( const char* name, const char* desc, THaApparatus* app )
  : Tracker(name,desc,app), fNPlanePartnerPairs(0), fMaxCorrMismatches(0),
    fMaxCorrNsigma(1.0), fMaxCorrDeltat(0.0), fminAdcToCheck(0.0)
{
  // Constructor
...
Int_t GEMTracker::ReadDatabase( const TDatime& date )
{
  ...
  DBRequest request[] = {
    ...
    { "3d_ampcorr_deltat",  &fMaxCorrDeltat,     kDouble, 0, 1 },
...
//_____________________________________________________________________________
Int_t GEMTracker::Candidate2Dmatch()
{
  double ADCasym, Delta_t;
  TClonesArray* fHitsX;
  TClonesArray* fHitsY;
  Int_t n2Dcombis = 0;
  //Matching 1D hits plane per plane for all combinations 

  const Rpvec_t& xplanes = (fProj[0])->GetListOfPlanes();
  
  for(Int_t i_plx = 0; i_plx<xplanes.size(); i_plx++){
    GEMPlane* planeX = (GEMPlane*)xplanes[i_plx];
    GEMPlane* planeY = (GEMPlane*)planeX->GetPartner();
    
    for(int i_x = 0; i_x<planeX->GetNhits(); i_x++){
      GEMHit* hitX = (GEMHit*)planeX->GetHit(i_x);
      for(int i_y = 0; i_y<planeY->GetNhits(); i_y++){
	GEMHit* hitY = (GEMHit*)planeY->GetHit(i_y);
	
	ADCasym = (hitX->GetADCsum()-hitY->GetADCsum())/(hitX->GetADCsum()+hitY->GetADCsum());
	Delta_t = hitX->GetPeaktime()-hitY->GetPeaktime();
	
	if(fabs(ADCasym)<fMaxCorrNsigma && fabs(Delta_t)<fMaxCorrDeltat){
	  //hopefully asking correlation for two variables completely suppresses the chance to store hits twice ?
	  fHitsX->AddLast(hitX);
	  fHitsY->AddLast(hitY);
	  n2Dcombis++;
	}
    ...
    //for(int i_pl = 0; i_pl< fPlanes)
    
    planeX->UpdateHitsList(fHitsX, true);
    planeY->UpdateHitsList(fHitsY, true);
  }//endl loop on planes;
		       ...
		       Rset_t& unique_found )
{
  ...
  for( Rvec_t::iterator itx = roads[0].begin(); itx != roads[0].end();
    ...
    const vector<std::pair<Road::Pvec_t, Double_t>>& xAllpoints = xroad->GetAllCandidatePoints();
	...
	continue;
      ...
      const vector<std::pair<Road::Pvec_t, Double_t>>& yAllpoints = yroad->GetAllCandidatePoints();
	...
	for(auto& pointsY : yAllpoints){
	  int temp_adc_x[nplanes], temp_adc_y[nplanes], temp_moduleID_x[nplanes], temp_moduleID_y[nplanes];
	  Double_t temp_matchval = 0.0;
	...
	// allowing missing hit in the other direction if the hit itself is small
	if( !ybits.TestBitNumber(xnum)){
	  if(static_cast<GEMHit*>(xp->hit)->GetADCsum() < 500){
	    nmatches++;
	  }else{
	  }
	  continue;
	...
	cout<<"x plane num: "<<xnum<<"   ";
	cout<<"xampl: "<<xampl<<" :yampl: "<<yampl<<"  module-> "<<xModuleID<<" "<<yModuleID<<endl;
	...
	cout<<"Good pair, nmatch: "<<nmatches<<endl;
	selected[0] = xroad;
	selected[1] = yroad;
	Add3dMatch( selected, matchval, combos_found, unique_found );
	++nfound;
