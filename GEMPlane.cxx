...
namespace TreeSearch {
  ...
  GEMPlane::GEMPlane( const char* name, const char* description,
    ...
    : Plane(name,description,parent),
      ...
      fOccupancy(0), 
      fClusOption(0), 
      fSampThr(0), fHiSampThr(0), fCentStripThr(0), fNshPeakshMin(0),
      fTimeBin(0), fSpreadHit(0), fTimeCst(0),
      fADCMap(0), fSamp(0), fHiSamp(0), fADCcent(0)
  {
    // Constructor
  ...
  Int_t GEMPlane::Begin( THaRunBase* run )
  {
    ...
    if( TestBit(kDoHistos) ) {
      ...
      hname.append("samp");
      fSamp = new TH1F(hname.c_str(), hname.c_str(), 100, -100, 900);
      hname = string(fPrefix);
      hname.append("hisamp");
      fHiSamp = new TH1F(hname.c_str(), hname.c_str(), 100, 0, 1000);
      hname = string(fPrefix);
      hname.append("adccent");
      fADCcent = new TH1F(hname.c_str(), hname.c_str(), 200, 0, 2000);
    }
#endif
    return 0;
  }
...
//_____________________________________________________________________________
  Int_t GEMPlane::DefaultClustering(){ // Code here the search of the hit candidates and fit their shape with a function
...
//_____________________________________________________________________________
  Double_t GEMPlane::IntegralHitPosFitFunc(double Amp, double Pos, double Time,
					   double x1, double x2)
  {
    assert(x2>x1);
    double t1 = TMath::Max(0., Time-fTimeCst);
    double t2 = fTimeBin*fMaxSamp;
    if(t1>t2)return 0;
    double Integral = Amp*( atan( (x2-Pos)/(fSpreadHit/fPitch) )-atan( (x1-Pos)/(fSpreadHit/fPitch) )  )/TMath::Pi();
    Integral*= ((t1-Time+2*fTimeCst)*exp(-(t1-(Time-fTimeCst))/fTimeCst)-(t2-Time+2*fTimeCst)*exp(-(t2-(Time-fTimeCst))/fTimeCst))/fTimeCst;
    
    return Integral;
}

//_____________________________________________________________________________
  Double_t GEMPlane::HitPosTimeFit(int centstrip, int peaksamp, int& ADCsum, double& amp, double& pos, double& time, int rangeL, int rangeR){
  //beware that this function returns a "position" in number of strips -  this position then needs to be converted back to 
  if(rangeR<0)rangeR=rangeL;
  const int nstrips = rangeL+rangeR+1;
  //assert(sizeof(ADC_ij)/sizeof(Int_t)==nstrips*NTIME_BINS);
  
  int stripmin = centstrip-rangeL;
  int stripmax = centstrip+rangeR+1;
  
  TH2D* h2D = new TH2D("h2D", "", nstrips, stripmin-0.5, stripmax-0.5, fMaxSamp, 0.0, fMaxSamp*fTimeBin);
  
  for(int i = 0; i<nstrips; i++){
    for(int j = 0; j<fMaxSamp; j++){
      //ADCsum+= fADCarray[stripmin+i][j];
      h2D->SetBinContent(i+1, j+1, fADCarray[stripmin+i][j]);
      h2D->SetBinError(i+1, j+1, 20.0);// should *have* been in DB
    }
  }
  ADCsum = h2D->Integral();
  //fit of the form:
  // ADC_ij = A * ( 1 / ( 1+(xi-x0)/sigma2 ) ) * ( (tj-t0)/tau exp ( (tj-t0)/tau ) )
  TF2* fpulsespread = new TF2("fpulsespread", 
			      //"TMath::Max( 0.0, [0]*( 1.0/(1+pow( (x-[1])/0.004, 2) ) )*(y-([2]-56.0))/56.0*TMath::Exp(-(y-([2]-56.0))/56.0) )",
			      Form("TMath::Max( 0., [0]*%1.1f*(1./(1.+pow(([1]-x)/%1.1f, 2)))/(TMath::Pi()*%1.1f)*(y-([2]-%1.1f))/pow(%1.1f, 2)*TMath::Exp(-(y-([2]-%1.1f))/%1.1f) )", fTimeBin, fSpreadHit/fPitch, fSpreadHit/fPitch, fTimeCst, fTimeCst, fTimeCst, fTimeCst),
			      stripmin, 
			      stripmax,
			      0.0, fTimeBin*fMaxSamp);
  //cout << fpulsespread->GetExpFormula() << endl;
  fpulsespread->SetParameter(1, centstrip);
  fpulsespread->SetParameter(2, (peaksamp+0.5)*fTimeBin);
  
  h2D->Fit(fpulsespread, "QR0");
  amp = fpulsespread->GetParameter(0);
  pos = fpulsespread->GetParameter(1);
  time = fpulsespread->GetParameter(2);
  
  //cout << " amp " << amp << " pos " << pos << " time " << time << endl;
  //h2D->Draw("lego2");h2D->SetMarkerSize(2);
  //fpulsespread->Draw("same");
  //cout << " function numerical integral " << fpulsespread->Integral(stripmin, time-TAU, stripmax, NTIME_BINS*TIME_BIN, 1.e-9) << endl;
  //cout << " hist sum " << h2D->Integral() << endl;
  double Chi2 = fpulsespread->GetChisquare();
  h2D->Delete();
  fpulsespread->Delete();
  return(Chi2);
}
  ...
  //_____________________________________________________________________________
  Int_t GEMPlane::ClusteringFitTemplate(){ // Code here the search of the hit candidates and fit their shape with a function
    ...
    int hi_strip = 0;
    int ADCsumHit = 0;
    double amplitude, position, time;
    bool sigshape;

    int RangeL, RangeR;
    // Int_t** ADC_ij = new Int_t*[5];
    // for(int i = 0; i<5; i++){
    //   ADC_ij[i] = new Int_t[fMaxSamp];
    // }
    //double Chi2 = 0;
    //double NDF = 0;
    for(int i_st = 1; i_st<fNelem-1; i_st++){
      //if(fADCarray[i_st].size())cout << " st "<< i_st; 
      if(fmStripModule[i_st]<0 || (fmStripModule[i_st]!=fmStripModule[i_st+1] || fmStripModule[i_st]!=fmStripModule[i_st-1]))continue;
      if(!fADCarray[i_st+1].size() || !fADCarray[i_st-1].size())continue;
      ...
      ADCsumHit = 0;
      hi_strip = -1;
      amplitude = position = time = 0;
      sigshape = false;
      
      for(int i_sp = 0; i_sp<fMaxSamp; i_sp++){
	ADC_cent+= fADCarray[i_st][i_sp];
	fSamp->Fill(fADCarray[i_st][i_sp]); 
	if( (fADCarray[i_st][i_sp]>fADCarray[i_st+1][i_sp] &&
	     fADCarray[i_st][i_sp]>fADCarray[i_st-1][i_sp]) && 
	    fADCarray[i_st][i_sp]>=fSampThr){
	  counter++;
	  if(fADCarray[i_st][i_sp]>=fHiSampThr){
	    if(1<=i_sp && i_sp<=4)sigshape = true;
	    if(fADCarray[i_st][i_sp]>maxsample){
	      hi_strip = i_sp;
	      maxsample = fADCarray[i_st][i_sp];
	    }
	  }
	}
      }
      fHiSamp->Fill(maxsample);
      fADCcent->Fill(ADC_cent);
      if(counter>=fNshPeakshMin && hi_strip>=0 && ADC_cent>fCentStripThr){
	RangeL = RangeR = 1;
	for(int i = 2; i<3; i++){
	  if(i_st+i<fNelem)if(fmStripModule[i_st+i]==fmStripModule[i_st])RangeR++;
	  if(i_st-i>=0)if(fmStripModule[i_st-i]==fmStripModule[i_st])RangeL++;
	}
	//NDF = fMaxSamp*(RangeL+RangeR+1)-3.0;
	//Chi2 = 
	HitPosTimeFit(i_st, hi_strip, ADCsumHit, amplitude, position, time, RangeL, RangeR);
	
	amplitude = IntegralHitPosFitFunc(amplitude, position, time, i_st-RangeL, i_st+RangeR+1);
	position = GetStart() + GetPitch()*(position-fDetMap->GetModule(fmStripModule[i_st])->first)+GetModuleOffsets(fmStripModule[i_st]);

	if(time<fPeakTimeMin || fPeakTimeMax<time)sigshape = false;
	if(!sigshape)continue;
	...
	if( !mc_data ) {
	  ...
	  theHit =
#endif
	    new( (*fHits)[nHits++] ) GEMHit( fmStripModule[i_st],
					     position,
					     ADCsumHit,
					     amplitude,
					     time,
	  ...
	  // Monte Carlo data
	  //if(fMCHitInfo[i_st].fSigType&0x1){
	  // in no bkgd, if one has mc hit info associated with strip candidate, 
	  // it is probably the max
	  //for(int i_sp = 0; i_sp<fMaxSamp; i_sp++)fSamp->Fill(fADCarray[i_st][i_sp]);
	  //fHiSamp->Fill(maxsample);
	  //fADCcent->Fill(ADC_cent);
	  //}
	  
	  double prim_ratio = 0.0, p_over_total = 0.0, b_over_total = 0.0;
	  double pos_sigma = 0.0, time_sigma = 0.0, adc_sigma = 0.0;
	  map<int,int> bg_registered;
	  Double_t mcpos = 0.0, mccharge = 0.0, mctime = kBig;
	  Int_t mctrack = 0, num_bg = 0;
	  
	  for(int i_st_ = i_st-RangeL; i_st_<=i_st+RangeR; i_st_++){
	    TSBSMCHitInfo& mc = fMCHitInfo[i_st_];
	    if(mc.fSigType&0x1){
	      for(int i_sp = 0; i_sp<fMaxSamp; i_sp++){
		prim_ratio+=mc.vClusterADC[i_sp][0];
	      }
	      if(mc.vClusterID.size()>1){
		for(int i_c=1;i_c<mc.vClusterID.size();i_c++){
		  //i_c<1,i_c=0 means primary hit....the first digitized cluster...bad
		  int cluster_id_tmp = mc.vClusterID[i_c];
		  if(bg_registered.find(cluster_id_tmp) == bg_registered.end()){// if this bg not registered
		    Double_t pos_tmp;
		    if(this->fName[2]=='x'){ 
		      pos_sigma += pow((mc.vClusterPos[i_c].X()-mc.vClusterPos[0].X()), 2);
		    }else if(this->fName[2]=='y'){
		      pos_sigma += pow((mc.vClusterPos[i_c].Y()-mc.vClusterPos[0].Y()), 2);
		    }else{
		      cout<<"ERROR in plane name!"<<endl;
		    }
		    
		    time_sigma += pow((mc.vClusterPeakTime[i_c]-mc.vClusterPeakTime[0]), 2);
		    adc_sigma += pow((mc.vClusterCharge[i_c]-mc.vClusterCharge[0]), 2);
		    bg_registered[cluster_id_tmp] = 0;
		  }//end if bg_registered
		}//end loop on cluster
	      }//if clustersize>0
	    ...
	    }//end if(mc.sigtype&0x1)
	      
	  }//endl loop
	  
	  mcpos /= static_cast<Double_t>(RangeL+RangeR+1);
	  prim_ratio /= static_cast<Double_t>(ADCsumHit);
	  
#ifndef NDEBUG
	  //cout  << "Plane " << GetName() << ": 1D Hit candidate no " << nHits << " " << fmStripModule[i_st] << " " << position << " " << ADCsumHit << " " << amplitude << " " << prim_ratio << p_over_total << " " << b_over_total << " " << pos_sigma  << " " << time_sigma << " " << adc_sigma << bg_registered.size() << " " << time << " " << RangeL+RangeR+1 << " " << i_st-RangeL << " " << i_st+RangeR << " 0 " << fResolution << " this " << mctrack << " " << mcpos << " " <<  mccharge << " " <<  mctime << " " << num_bg << endl;
	    theHit =
#endif
	    new( (*fHits)[nHits++] ) MCGEMHit( fmStripModule[i_st],
					       position,
					       ADCsumHit,
					       amplitude,
					       prim_ratio,
  ...
  Int_t GEMPlane::GEMDecode( const THaEvData& evData )
  {
...
#ifdef TESTCODE
    if( TestBit(kDoHistos) )
      assert( fHitMap != 0 and fADCMap != 0 and fSamp != 0 and fHiSamp != 0 and fADCcent != 0);
#endif
	...
	if(fClusOption>0){
	  fADCarray[istrip].resize(fMaxSamp);
	}
	...
	for( Int_t isamp = 0; isamp < nsamp; ++isamp ) {
	  ...
	  if(fClusOption>0){
	    assert(isamp<fMaxSamp);// please pass that...
	    fADCarray[istrip][isamp] = fsamp;
	  }
	}
	...
	for( Int_t ichan = 0; ichan < nchan; ++ichan ){
	  ...
	  fmStripModule[istrip] = -1;
	  ADCsum = 0;
	  assert(fADCarray[istrip].size()==fMaxSamp);
	  for(UInt_t isamp=0; isamp<fMaxSamp; isamp++){
	    ADCsum+=fADCarray[istrip][isamp];
	    //fADCarray[ichan][isamp]-=cMode[ichan/fcModeSize][isamp];
	  }
	  //zero suppression
	  if(ADCsum>=fMinAmpl || fMinAmpl<=0){
	    fNhitStrips++;
	    fmStripModule[istrip] = moduleID;
	    //if(moduleID==0)cout << " " << istrip;
	  }else{
	    //if(moduleID==0)cout << " " << istrip;
	    fADCarray[istrip].clear();
	  }
	}
      }else{//Decoding for default clustering
	// subtracting cMode[][]
...
//#define PRINTCLUSTER_TO_FILE
    ...
    cout << "Plane " << GetName() << ": nHits = " << nHits << endl;
  ...
  */
  ...
  Int_t GEMPlane::ReadDatabase( const TDatime& date )
  {
	...
	{ "clusopt",        &fClusOption,     kInt,     0, 1, gbl },
	{ "samp_thr", &fSampThr,      kInt,     0, 1, gbl },
	{ "hisamp_thr", &fHiSampThr,      kInt,     0, 1, gbl },
	{ "centstp_thr",  &fCentStripThr,      kInt,     0, 1, gbl },
	{ "n_shpeaksh_min", &fNshPeakshMin,      kInt,     0, 1, gbl },
	{ "timebin", &fTimeBin,      kDouble,  0, 1, gbl },
	{ "spreadhit", &fSpreadHit,      kDouble,  0, 1, gbl },
	{ "pulsetimecst", &fTimeCst,      kDouble,  0, 1, gbl },
	{ 0 }
      };
